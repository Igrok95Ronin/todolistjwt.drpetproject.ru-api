...
# Сборка приложения
RUN CGO_ENABLED=0 GOOS=linux go build -o /todolistjwt-drpetproject ./cmd/todolistjwt
...
COPY --from=build /todolistjwt-drpetproject /usr/local/src/todolistjwt-drpetproject
COPY ./config.yml ./
